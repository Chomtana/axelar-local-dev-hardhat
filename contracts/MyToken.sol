...
contract ChomToken is ERC20, ERC20Burnable, Ownable, AxelarExecutable {
    ...
    function setDestinationAddress(
        ...
        destinationAddressMapping[
            keccak256(abi.encode(destinationChain))
        ] = destinationAddress;
    ...
    function bridge(
        ...
        string memory destinationAddress = destinationAddressMapping[
            keccak256(abi.encode(destinationChain))
        ];

        require(bytes(destinationAddress).length > 0, "Destination address not set");
